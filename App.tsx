...
const App = () => {
  ...
  const [recordedSizeBytes, setRecordedSizeBytes] = useState<number | null>(
    null,
  );
  ...
  const loadWhisperModel = useCallback(async (): Promise<WhisperContext> => {
    // Keep one Whisper context alive and reuse it across recordings.
    if (whisperContextRef.current) {
    ...
    setModelProgress(0);
    setStatusMessage('Preparing Whisper small model...');

    try {
      const modelPath = await downloadModel('small', progress => {
  ...
  const transcribeRecording = useCallback(
    async (filePath: string) => {
      // Transcription runs immediately after save, using the cached model/context when available.
      setIsTranscribing(true);
      setTranscribeProgress(0);
      setStatusMessage('Transcribing with Whisper small...');
      ...
      try {
        ...
        setStatusMessage('Recording has been saved and transcribed.');
      } catch (error) {
        setStatusMessage(
  ...
  const startRecording = useCallback(async () => {
    // Prevent overlapping record/transcribe/model-init operations.
    ...
    try {
      ...
      // We track the intended final path because the native module may return a different source path.
  ...
  const stopRecording = useCallback(async () => {
    ...
    try {
      ...
      // If the recorder wrote elsewhere, move it into our managed audio folder.
      ...
      // Auto-transcribe as part of the same stop flow.
  ...
  const modelProgressText = useMemo(() => {
    ...
    return `Model: small (${Math.round(modelProgress * 100)}%)`;
  }, [modelProgress]);
